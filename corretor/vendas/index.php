<?php
// corretor/vendas/index.php - Painel de Vendas do Corretor
...
try {
    ...
    error_log("Erro crítico na inicialização do DB em corretor/vendas/index.php: " . $e->getMessage());
...
// --- KPIs de Vendas para o Corretor ---
$total_vendas_periodo = 0;
$valor_total_vendido_periodo = 0;
$comissao_total_recebida_periodo = 0;
...
try {
    $sql_kpis = "
        SELECT
            ...
            COALESCE(SUM(r.comissao_corretor), 0) AS comissao_total
        FROM
            reservas r
        WHERE
            r.corretor_id = ?
            AND r.status = 'vendida'
            AND DATE(r.data_ultima_interacao) BETWEEN ? AND ?
    ";
    $params_kpis = [$corretor_id, $filter_start_date, $filter_end_date];
    $types_kpis = "iss";
    ...
    $comissao_total_recebida_periodo = $result_kpis['comissao_total'] ?? 0;
    ...
    error_log("Erro ao carregar KPIs de vendas do corretor: " . $e->getMessage());
...
try {
    $sql_vendas = "
        SELECT
            ...
            COALESCE(uiu.nome, 'Sistema/Não Identificado') AS usuario_ultima_interacao_nome -- Quem finalizou (Admin)
        FROM
            reservas r
        JOIN
        ...
        WHERE
            ...
            AND DATE(r.data_ultima_interacao) BETWEEN ? AND ?
    ";
    $params = [$corretor_id, $filter_start_date, $filter_end_date];
    $types = "iss";

    // Aplica filtro de busca (se houver)
    if ($search_term) {
        $sql_vendas .= " AND (cl.nome LIKE ? OR cl.cpf LIKE ? OR e.nome LIKE ? OR u.numero LIKE ?)";
        $params[] = '%' . $search_term . '%';
        $params[] = '%' . $search_term . '%';
        $params[] = '%' . $search_term . '%';
        $params[] = '%' . $search_term . '%';
    ...
    error_log("Erro ao carregar lista de vendas do corretor: " . $e->getMessage());
    $errors[] = "Ocorreu um erro ao carregar suas vendas: " . $e->getMessage();
}
...
<div class="admin-content-wrapper">
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card">
            <span class="kpi-label">Comissão Recebida</span>
            <span class="kpi-value"><?php echo format_currency_brl($comissao_total_recebida_periodo); ?></span>
            <small>No período selecionado</small>
        </div>
        <div class="kpi-card">
    ...
    <div class="admin-controls-bar">
        <form id="vendasFiltersForm" method="GET" action="<?php echo BASE_URL; ?>corretor/vendas/index.php">
        ...
        <table class="admin-table" id="vendasTable">
            <thead>
                <tr>
                    ...
                    <th data-sort-by="comissao_corretor">Minha Comissão <i class="fas fa-sort"></i></th>
            ...
            <tbody>
                <?php if (empty($vendas)): ?>
                    <tr><td colspan="9" style="text-align: center;">Nenhuma venda encontrada com os filtros aplicados.</td></tr>
                <?php else: ?>
                    <?php foreach ($vendas as $venda): ?>
                        <tr>
                            ...
                            <td class="admin-table-actions">
                                <a href="<?php echo BASE_URL; ?>corretor/reservas/detalhes.php?id=<?php echo htmlspecialchars($venda['venda_id']); ?>" class="btn btn-info btn-sm">Ver Detalhes</a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
